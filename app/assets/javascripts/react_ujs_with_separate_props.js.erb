...
var ReactRailsUJS = {
  ...
  // helper method to find the <script> tag with the props
  findPropsScript: function(searchSelector, propsId) {
    var classNameAttr = ReactRailsUJS.PROPS_ID_ATTR;
    // we will use fully qualified paths as we do not bind the callbacks
    var selector, parent;

    switch (typeof searchSelector) {
      case 'undefined':
        selector = 'script[' + classNameAttr + '="' + propsId + '"' + ']';
        parent = document;
        break;
      case 'object':
        selector = 'script[' + classNameAttr + '="' + propsId + '"' + ']';
        ...
        break;
      case 'string':
        selector = searchSelector + 'script[' + classNameAttr + '="' + propsId + '"' + '], ' +
                   searchSelector + ' script[' + classNameAttr + '="' + propsId + '"' + ']';
        parent = document;
        break
      default:
        break;
    ...
    if (ReactRailsUJS.jQuery) {
      return ReactRailsUJS.jQuery(selector, parent)[0];
    } else {
  ...
  mountComponents: function(searchSelector) {
    ...
    for (var i = 0; i < nodes.length; ++i) {
      ...
      var propsId = node.getAttribute(ujs.PROPS_ID_ATTR);
      var propsJson = propsId && ujs.findPropsScript(searchSelector, propsId).text;
